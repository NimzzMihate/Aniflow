...
CREATE POLICY "Users can insert history" ON public.watch_history FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update history" ON public.watch_history FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete history" ON public.watch_history FOR DELETE USING (auth.uid() = user_id);
